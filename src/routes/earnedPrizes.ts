...
earnedPrizeRoutes.get('/:id', authenticate, async (req: AuthRequest, res, next) => {
  try {
    ...
    const formatted = {
      ...
      studentCount: earnedPrize.class?.studentCount ?? earnedPrize.teacher?.studentCount ?? 0,
      schoolId: schoolIdForRow,
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const formatted = {
        ...
        studentCount: updatedWithRelations?.class?.studentCount ?? updatedWithRelations?.teacher?.studentCount ?? 0,
        schoolId: schoolIdForRow,
